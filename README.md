# NATS01 - NATS Client для управления записью

Python-клиент для взаимодействия с Rust-приложением управления записью через NATS.

## Описание

Проект предоставляет асинхронный клиент для отправки команд управления записью аудио/видео через NATS message broker. Поддерживает команды `start`, `stop`, `status`.
